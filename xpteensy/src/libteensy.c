...
int init_teensy() {
  ...
  for (tee=0;tee<MAXTEENSYS;tee++) {
	...
	if (teensy[tee].inputs_isinput[input] == 1) {
	  if (verbose > 0) printf("Teensy %i Pin %i Initialized as Input \n",
				  tee,input);
	  teensySendBuffer[p*4+5] = 0x03;
	  teensySendBuffer[p*4+4] = input;
	  teensySendBuffer[p*4+6] = 0x00;
	  teensySendBuffer[p*4+7] = 0x00;
	  p++;
	...
	if (p == MAXPACKETSEND) {
      ...
      for (input=0;input<teensy[tee].nanaloginputs;input++) {
	if (teensy[tee].analoginputs_isinput[input] == 1) {
	  if (verbose > 0) printf("Teensy %i Analog Input %i initialized to send data \n",
				  tee,input);
	  teensySendBuffer[p*4+5] = 0x04;
	  teensySendBuffer[p*4+4] = input;
	  teensySendBuffer[p*4+6] = 0x00;
	  teensySendBuffer[p*4+7] = 0x00;
	  p++;
