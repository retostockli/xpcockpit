...
namespace OpenGC {
  ...
  void A320ClockComponent::Render () {
	...
	int *cold_dark_clock = link_dataref_int ("xpserver/cold_and_dark");
	if (*cold_dark_clock == INT_MISS) coldDarkClk = 2; else coldDarkClk = *cold_dark_clock;
	if (coldDarkClk == 0) {

        // Used X-Plane DataRefs
//        int *cy = link_dataref_int("sim/cockpit2/clock_timer/current_year");        // not yet defined in sim & not used here
...
//        int *eth = link_dataref_int("sim/cockpit2/clock_timer/elapsed_time_hours"); // not used here
    ...
    switch (etState) {
      ...
      case 2:
        ...
        break;
printf (buff, "trt: %5.3f, etStart: %02d, etVal: %02d, etStop: %02d, etState: %d\n", trt, etStart, etVal, etStop, etState);
      }
