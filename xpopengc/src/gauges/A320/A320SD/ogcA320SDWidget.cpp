...
//===========================The Render function===============================
  ...
  void A320SDWidget::Render()
  {
	...
	int *cold_dark_sd = link_dataref_int ("xpserver/cold_and_dark");
	if (*cold_dark_sd == INT_MISS) coldDarkSd = 2; else coldDarkSd = *cold_dark_sd;
	if (coldDarkSd == 0) {
...
//	AirbusFBW/FuelPumpOHPArray	int[16] 4 pos (image SD_FUEL_Pump)
...
//	AirbusFBW/FuelXFVSDArray	int[8] 5 pos (image SD_FUEL_XFV)
    int fuelXFV = 0;
    int *fuel_xfv = link_dataref_int_arr ("AirbusFBW/FuelXFVSDArray", 8, 0);
    if (*fuel_xfv != INT_MISS) fuelXFV = *fuel_xfv;

//	AirbusFBW/FuelTVSDArray	int[8] 2 pos (1, 2: image SD_FUEL_TV_L; 3, 4: image SD_FUEL_TV_R)
...
//	AirbusFBW/ENGFuelLPValveArray	int[4] 4 pos (image SD_LPFuelValve)
...
//	sim/cockpit2/fuel/fuel_quantity	float[9]	n	kgs	Indicated fuel level per tank
...
//	sim/cockpit2/engine/indicators/fuel_flow_kg_sec	float[8]	n	kilograms/second	FF, kilograms per second.
