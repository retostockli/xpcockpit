/*=========================================================================
  ...
  Module:  $RCSfile: ogcVOR_Cntr.cpp,v $
...
#include "ogcVOR_Cntr.h"
#include <Source/DataSources/ogcDataSource.h>

#define width 1.75
int timer;

ogcVOR_Cntr::ogcVOR_Cntr()
{
  printf("ogcVOR_Cntr constructed\n");
...
void ogcVOR_Cntr::Render()
{
  ...
  //If we are in the ILS mode lets draw the glideslope markers
  
  if ( m_pDataSource->Nav1_Has_GS ) {
  	glColor3ub( 210, 5,  210 );  // color is magenta
		double gs_dev = m_pDataSource->Nav1_Glideslope_Needle;
		glBegin(GL_POLYGON);
		glVertex2f( 79, gs_dev ); 	

		glVertex2f( 83, gs_dev+4 );
		glVertex2f( 87, gs_dev );
		glVertex2f( 83, gs_dev-4 );
		glEnd();	
		...
		m_pFontManager->Print( 83, 30, "O", m_Font);
		m_pFontManager->Print( 83, 15, "O", m_Font);
		m_pFontManager->Print( 83, -16, "O", m_Font);
		m_pFontManager->Print( 83, -31, "O", m_Font);
		}
		// draw in white
...
// Need to add a check if EFIS switch is in VOR or ADF position green=vor blue=adf
 ...
 if ( m_pDataSource->Adf1_Valid ) glColor3ub( 10, 100, 255 ); else glColor3ub( 0, 255, 100);
 glBegin(GL_LINES);
  glVertex2f( 0, 60 );
  glVertex2f( 0, 72 );
  glVertex2f( 0, -60 );
 ...
 glRotated( -left_bearing, 0, 0, -1 );
  ...
  if ( m_pDataSource->Adf2_Valid ) glColor3ub( 10, 100, 255);
else glColor3ub( 0, 255, 100);
