/* This is the xpclient code. It is a sample client for the idea of an Open Source Data Interface 
   ...
   simulator, discussed here by the creators of the XHSI code:
   http://forums.x-plane.org/index.php?showtopic=65563

   This code here and the related X-Plane plugin (xpserver) is laregly based on the server/client
   data exchange xpserver/xpiocards found on http://xpiocards.sourceforge.net.
  ...
  for (;;)
	...
	*custom = 1;
	...
	printf("FMC1: XXX%sXXX\n",fmc1);
      ...
      /* run usbiocards data exchange (usb and tcp/ip) every 10000 microseconds */
